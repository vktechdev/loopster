...
class MyServiceWithSyncedWatchdog(softirq.SoftIrqService):
...
def main():
    ...
    etcd_config = clients.Config(
        endpoints=["http://127.0.0.1:2379/"],
        namespace="example",
        timeout=10,
    )
    watchdog = etcd.WatchDogEtcd(heartbeat_timeout=10,
                                 etcd_config=etcd_config,
                                 lock_key='watchdog')

    hub.add_service(svc_class=MyServiceWithSyncedWatchdog,
